...
static bool s_recording_offline;              /* 断连本地继续录音时用深色背景（design §7） */
...
/* 上传进度 widget（仅 UPLOADING 场景显示） */
#define UI_STATUS_FILE_NAME_MAX 64
static lv_obj_t *s_progress_bar;
static lv_obj_t *s_progress_sub_label;
static int s_progress_percent;
static char s_progress_file[UI_STATUS_FILE_NAME_MAX];
static uint64_t s_progress_sent;
static uint64_t s_progress_total;
...
static void render_scene_locked(ui_status_icon_scene_t scene, const char *status, const char *hint)
{
    ...
    const bool resting = scene == UI_STATUS_ICON_RECORDING && s_recording_offline;
    const bool pairing = scene == UI_STATUS_ICON_PAIRING || scene == UI_STATUS_ICON_BOOT;
    const bool error = scene == UI_STATUS_ICON_ERROR;
    const bool uploading = scene == UI_STATUS_ICON_UPLOADING;
    ...
    /* 上传进度：仅 UPLOADING 场景显示；其他场景隐藏。 */
    if (s_progress_bar != NULL) {
        lv_obj_set_style_bg_opa(s_progress_bar, LV_OPA_COVER, LV_PART_INDICATOR);
        lv_obj_set_style_bg_color(s_progress_bar, lv_color_hex(0x55c98a), LV_PART_INDICATOR);
    ...
    if (s_progress_sub_label != NULL) {
        if (uploading) {
            lv_label_set_text_fmt(s_progress_sub_label, "%s\n%llu/%llu B",
                                  s_progress_file,
                                  (unsigned long long)s_progress_sent,
                                  (unsigned long long)s_progress_total);
            lv_obj_clear_flag(s_progress_sub_label, LV_OBJ_FLAG_HIDDEN);
        } else {
            lv_obj_add_flag(s_progress_sub_label, LV_OBJ_FLAG_HIDDEN);
        }
    }
    /* UPLOADING：status label 上移到图标下方显示大号百分比；其他场景恢复原位。 */
    if (uploading) {
        lv_obj_align(s_status_label, LV_ALIGN_TOP_MID, 0, 158);
        lv_label_set_text_fmt(s_status_label, "%d%%", s_progress_percent);
    } else {
...
{
    /* 息屏(dim)时仍渲染当前场景（屏幕已调暗），不再用独立的 sleeping/resting 场景 */
    render_scene_locked(s_scene, s_status_text, s_hint_text);
}

static void create_status_ui(void)
{
    const board_lcd_config_t *lcd = board_hal_lcd_config();
    ...
    lv_obj_set_width(s_status_label, lcd->h_res - 16);
    lv_obj_set_style_text_align(s_status_label, LV_TEXT_ALIGN_CENTER, 0);
    lv_obj_align(s_status_label, LV_ALIGN_TOP_MID, 0, 168);

    s_hint_label = lv_label_create(s_screen);
    lv_label_set_long_mode(s_hint_label, LV_LABEL_LONG_WRAP);
    lv_obj_set_width(s_hint_label, lcd->h_res - 16);
    ...
    lv_obj_set_style_bg_color(s_progress_bar, lv_color_hex(0xeaded2), 0);
    lv_obj_set_style_border_width(s_progress_bar, 0, 0);
    lv_bar_set_range(s_progress_bar, 0, 100);
    lv_bar_set_value(s_progress_bar, 0, LV_ANIM_OFF);
    lv_obj_add_flag(s_progress_bar, LV_OBJ_FLAG_HIDDEN);
    lv_obj_align(s_progress_bar, LV_ALIGN_TOP_MID, 0, 188);

    s_progress_sub_label = lv_label_create(s_screen);
    lv_label_set_long_mode(s_progress_sub_label, LV_LABEL_LONG_CLIP);
    lv_obj_set_width(s_progress_sub_label, 288);
    lv_obj_set_style_text_align(s_progress_sub_label, LV_TEXT_ALIGN_CENTER, 0);
    lv_obj_set_style_text_font(s_progress_sub_label, &lv_font_montserrat_10, 0);
    lv_obj_set_style_text_color(s_progress_sub_label, lv_color_hex(0x7f7180), 0);
    lv_obj_add_flag(s_progress_sub_label, LV_OBJ_FLAG_HIDDEN);
    lv_obj_align(s_progress_sub_label, LV_ALIGN_TOP_MID, 0, 204);
...
{
    const board_lcd_config_t *lcd = board_hal_lcd_config();
    ESP_RETURN_ON_FALSE(lcd != NULL, ESP_ERR_INVALID_STATE, TAG, "lcd config unavailable");
    ...
    const ledc_channel_config_t backlight_channel = {
        ...
        .flags.output_invert = lcd->backlight_invert ? 1 : 0,
    };
    ESP_RETURN_ON_ERROR(ledc_channel_config(&backlight_channel), TAG, "configure backlight channel");

    spi_bus_config_t bus_config = {
        .sclk_io_num = lcd->sck_gpio,
        .mosi_io_num = lcd->mosi_gpio,
        .miso_io_num = -1,
        .quadwp_io_num = -1,
        .quadhd_io_num = -1,
        .max_transfer_sz = lcd->h_res * LVGL_DRAW_BUF_LINES * sizeof(lv_color16_t),
    };
    ...
    ESP_RETURN_ON_ERROR(esp_lcd_new_panel_io_spi((esp_lcd_spi_bus_handle_t)lcd->host,
                                                 &io_config, &io),
                        TAG, "create lcd panel io");

    esp_lcd_panel_handle_t panel = NULL;
    esp_lcd_panel_dev_config_t panel_config = {
        .reset_gpio_num = lcd->rst_gpio,
    ...
    /* 立创派：panel_reset 后通过 PCA9557 IO0 拉低 LCD_CS，再 panel_init。
     * 对应 xiaozhi-esp32 lichuang_dev_board.cc 的初始化顺序。
     */
    ESP_RETURN_ON_ERROR(lichuang_board_lcd_enable_cs(), TAG, "enable lcd cs via pca9557");
    ESP_RETURN_ON_ERROR(esp_lcd_panel_init(panel), TAG, "init panel");
    ESP_RETURN_ON_ERROR(esp_lcd_panel_invert_color(panel, lcd->color_invert),
                        TAG, "invert panel colors");
    ESP_RETURN_ON_ERROR(esp_lcd_panel_swap_xy(panel, lcd->swap_xy),
                        TAG, "swap xy");
    ESP_RETURN_ON_ERROR(esp_lcd_panel_mirror(panel, lcd->mirror_x, lcd->mirror_y),
                        TAG, "mirror panel");
    ...
    lv_init();
    s_display = lv_display_create(lcd->h_res, lcd->v_res);
    ESP_RETURN_ON_FALSE(s_display, ESP_ERR_NO_MEM, TAG, "create lvgl display");

    const size_t draw_buffer_size = lcd->h_res * LVGL_DRAW_BUF_LINES * sizeof(lv_color16_t);
    void *buf1 = spi_bus_dma_memory_alloc(lcd->host, draw_buffer_size, 0);
    void *buf2 = spi_bus_dma_memory_alloc(lcd->host, draw_buffer_size, 0);
...
void ui_status_set_advertising(void)
{
    ...
    set_scene(UI_STATUS_ICON_PAIRING, "Pairing", "Search via recorder");
}
...
void ui_status_set_pairing(const char *device_name)
{
    ...
    set_scene(UI_STATUS_ICON_PAIRING, "Pairing", device_name ? device_name : "luyinbi-esp");
}

void ui_status_set_connecting(void)
{
    ESP_LOGD(TAG, "connecting");
    set_scene(UI_STATUS_ICON_PAIRING, "Connecting", "Handshake");
}

void ui_status_set_idle(void)
{
    ESP_LOGD(TAG, "idle");
    set_scene(UI_STATUS_ICON_IDLE, "Ready", "Press BOOT to record");
}

void ui_status_set_busy(const char *status, const char *hint)
{
    ESP_LOGD(TAG, "busy status=%s hint=%s", status ? status : "", hint ? hint : "");
    set_scene(UI_STATUS_ICON_TRANSCRIBING,
...
{
    ESP_LOGD(TAG, "file transfer percent=%d", percent);
    if (percent < 0) {
        percent = 0;
    } else if (percent > 100) {
        percent = 100;
    ...
    strlcpy(s_progress_file, file_name ? file_name : "", sizeof(s_progress_file));
    s_progress_sent = sent;
    s_progress_total = total;
    set_scene(UI_STATUS_ICON_UPLOADING, "", NULL);
}

void ui_status_set_link_down(void)
{
    ESP_LOGD(TAG, "link down");
    set_scene(UI_STATUS_ICON_PAIRING, "Waiting", "Waiting for device");
}
...
{
    ESP_LOGD(TAG, "recording elapsed=%s offline=%d", elapsed ? elapsed : "", offline);
    s_recording_offline = offline;
    /* 断连本地继续录音时 hint 标注离线（design §7）；无中文字库，用英文 */
    set_scene(UI_STATUS_ICON_RECORDING,
              elapsed && elapsed[0] ? elapsed : "00:00",
              offline ? "Recording Offline" : "Speak now");
}
